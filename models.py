...
class Anime(Base):
    ...
    # MUHIM: sqlite_autoincrement=True — SQLite'ga o'chirilgan animening ID'sini
    # HECH QACHON yangi animega qayta bermaslikni buyuradi. Aks holda o'chirilgan
    # anime'ning "etim" qolib ketgan qismlari (episodes) yangi animega yopishib qolishi mumkin.
    ...
    is_premium: Mapped[bool] = mapped_column(Boolean, default=False)
    # Agar to'ldirilgan bo'lsa — shu sanadan keyin anime AVTOMATIK hammaga (tekin) ochiladi.
    # None bo'lsa va is_premium=True bo'lsa — anime doimiy Premium bo'lib qoladi.
    premium_free_at: Mapped[datetime.datetime] = mapped_column(DateTime, nullable=True)
    downloads: Mapped[int] = mapped_column(Integer, default=0)
    cover_file_id: Mapped[str] = mapped_column(String(255), default="")
    trailer_file_id: Mapped[str] = mapped_column(String(255), default="")  # tanishtiruv (trailer) videosi
...
class PostChannel(Base):
    """Post joylash uchun ilgari ishlatilgan/saqlangan kanallar ro'yxati.
    Majburiy obuna Channel jadvalidan farqli — bu shunchaki 'tez tanlash' ro'yxati."""
    __tablename__ = "post_channels"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    chat_id: Mapped[str] = mapped_column(String(64), unique=True)   # -1001234567890 yoki @kanal
    title: Mapped[str] = mapped_column(String(255), default="")
...
class ResponsibleAdmin(Base):
    """Har bir soha (reklama, yordam, premium) uchun mas'ul (javobgar) adminni belgilaydi.
    Faqat bosh adminlar (config.ADMIN_IDS) buni o'zgartira oladi. Agar yozuv bo'lmasa,
    kod ichida standart bo'yicha 1-bosh admin ishlatiladi."""
    __tablename__ = "responsible_admins"

    role: Mapped[str] = mapped_column(String(20), primary_key=True)  # "ads" | "help" | "premium"
    admin_id: Mapped[int] = mapped_column(BigInteger)


class ExternalLink(Base):
    """Telegramdan tashqaridagi linklar (Instagram, YouTube va h.k.).
    Faqat YANGI foydalanuvchiga /start'da bir marta ko'rsatiladi, tekshirilmaydi —
    foydalanuvchi shunchaki "Davom etish" tugmasini bossa yetarli."""
