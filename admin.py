...
async def post_code(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"📤 Endi post qaysi KANALGA joylansin? Kanal ID yoki @username yuboring "
        f"(masalan: <code>@mening_kanalim</code> yoki <code>-1001234567890</code>).\n\n"
        f"❗️ Bot o'sha kanalda ADMIN bo'lishi va post yuborish huquqiga ega bo'lishi kerak."
    )
...
async def post_channel(message: Message, state: FSMContext):
    if message.text == CANCEL:
        await admin_cancel(message, state); return
    await state.update_data(post_channel=message.text.strip())
    await message.answer(
...
async def post_photo(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        await message.answer(
            "✅ Post kanalga muvaffaqiyatli joylandi — tugma bilan birga!",
            reply_markup=admin_kb.admin_panel_kb()
        )
    except Exception as e:
        await message.answer(
            f"❌ Postni kanalga joylab bo'lmadi.\n"
            f"Bot o'sha kanalda ADMIN ekanini va post yuborish huquqi borligini tekshiring.\n\n"
