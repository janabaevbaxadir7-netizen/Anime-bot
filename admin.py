...
# ══════════════════ BAZANI ZAHIRALASH / TIKLASH (faqat super admin) ══════════════════
# MUHIM: bu vaqtinchalik ko'chirish qurollari — bir serverdan (masalan eski Railway)
# ma'lumotlarni olib, boshqasiga (yangi Railway) ko'chirish uchun ishlatiladi.
# DB_PATH "sqlite+aiosqlite:///PATH" ko'rinishida bo'lgani uchun haqiqiy fayl
# manzilini shu yerdan olib beramiz.
...
@router.message(Command("backup_db"), IsSuperAdmin())
async def backup_db(message: Message, bot: Bot):
    ...
    if not db_file or not os.path.exists(db_file):
        await message.answer(
            "⚠️ Baza fayli topilmadi (SQLite ishlatilmayotgan bo'lishi mumkin, "
            "yoki DB_PATH boshqacha sozlangan)."
        )
        return
    ...
    await message.answer_document(
        ...
        caption=(
            "📦 Baza zahira nusxasi.\n\n"
            "Yangi serverga o'tkazish uchun: shu faylni yangi joyda "
            "<code>/restore_db</code> buyrug'iga javoban qayta yuboring."
        ),
    )


@router.message(Command("restore_db"), IsSuperAdmin())
async def restore_db_prompt(message: Message):
    await message.answer(
        "♻️ Zahira faylni (masalan <code>animebot.db</code>) hozir DOCUMENT sifatida yuboring.\n\n"
        "⚠️ Diqqat: joriy baza fayli UST-USTIGA yoziladi va butunlay almashtiriladi!"
    )


@router.message(F.document, IsSuperAdmin())
async def restore_db_receive(message: Message, bot: Bot):
    # Faqat .db fayllarni qabul qilamiz — tasodifiy boshqa hujjatlarni bazaga
    # yozib qo'ymaslik uchun.
    if not (message.document.file_name or "").endswith(".db"):
        return

    db_file = _sqlite_file_path()
    if not db_file:
        await message.answer("⚠️ SQLite ishlatilmayapti, avtomatik tiklab bo'lmaydi.")
        return

    file = await bot.get_file(message.document.file_id)
    await bot.download_file(file.file_path, destination=db_file)
    await message.answer(
        "✅ Baza muvaffaqiyatli tiklandi!\n\n"
        "⚠️ O'zgarishlar to'liq kuchga kirishi uchun botni qayta ishga tushiring "
        "(Railway'da: Deployments → Redeploy)."
    )
...
async def _create_anime_and_prompt_episodes(message: Message, state: FSMContext, title: str, description: str, cover_file_id: str = "", trailer_file_id: str = ""):
    ...
    if trailer_file_id:
        await crud.update_anime_trailer(anime.id, trailer_file_id)
...
async def _prompt_trailer_step(message: Message, state: FSMContext, title: str, description: str, cover_file_id: str = ""):
    await state.update_data(title=title, description=description, cover_file_id=cover_file_id)
    await message.answer(
        "🎥 Anime uchun <b>tanishtiruv (trailer) videosi</b>ni yuboring.\n"
        "Kerak bo'lmasa ➡️ Skip bosing:",
        reply_markup=admin_kb.skip_kb()
    )
    await state.set_state(AdminStates.waiting_new_trailer)
...
async def am_new_desc_photo(message: Message, state: FSMContext):
    ...
    await _prompt_trailer_step(message, state, data["title"], description, message.photo[-1].file_id)
...
async def am_new_desc(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    await _prompt_trailer_step(message, state, data["title"], description)


@router.message(AdminStates.waiting_new_trailer, F.video)
async def am_new_trailer_video(message: Message, state: FSMContext):
    data = await state.get_data()
    await _create_anime_and_prompt_episodes(
        message, state, data["title"], data.get("description", ""),
        data.get("cover_file_id", ""), message.video.file_id
    )


@router.message(AdminStates.waiting_new_trailer)
async def am_new_trailer_text(message: Message, state: FSMContext):
    if message.text == CANCEL:
        await admin_cancel(message, state); return
    if message.text != SKIP:
        await message.answer("🎥 Iltimos, video yuboring yoki ➡️ Skip bosing.")
        return
    data = await state.get_data()
    await _create_anime_and_prompt_episodes(
        message, state, data["title"], data.get("description", ""), data.get("cover_file_id", "")
    )
...
async def am_view(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        f"🎥 Trailer: {'✅ bor' if anime.trailer_file_id else '❌ yo\u02bbq'}",
...
@router.callback_query(F.data.startswith("aedit_trailer:"))
async def aedit_trailer_start(callback: CallbackQuery, state: FSMContext):
    anime_id = int(callback.data.split(":")[1])
    await state.update_data(edit_anime_id=anime_id)
    await callback.message.answer(
        "🎥 Yangi tanishtiruv (trailer) videosini yuboring.\n"
        "Trailerni butunlay olib tashlamoqchi bo'lsangiz — ➡️ Skip bosing:",
        reply_markup=admin_kb.skip_kb()
    )
    await state.set_state(AdminStates.waiting_edit_trailer)
    await callback.answer()


@router.message(AdminStates.waiting_edit_trailer, F.video)
async def aedit_trailer_video(message: Message, state: FSMContext):
    data = await state.get_data()
    await crud.update_anime_trailer(data["edit_anime_id"], message.video.file_id)
    await state.clear()
    await message.answer("✅ Trailer yangilandi!", reply_markup=admin_kb.admin_panel_kb())


@router.message(AdminStates.waiting_edit_trailer)
async def aedit_trailer_text(message: Message, state: FSMContext):
    if message.text == CANCEL:
        await admin_cancel(message, state); return
    if message.text != SKIP:
        await message.answer("🎥 Iltimos, video yuboring yoki ➡️ Skip bosing.")
        return
    data = await state.get_data()
    await crud.update_anime_trailer(data["edit_anime_id"], "")
    await state.clear()
    await message.answer("✅ Trailer olib tashlandi!", reply_markup=admin_kb.admin_panel_kb())
...
ROLE_LABELS = {"ads": "Reklama", "help": "Yordam", "premium": "Premium"}


async def _ads_menu_text_and_kb(uid: int):
    links = await crud.get_all_external_links()
    is_super = uid in config.ADMIN_IDS
    resp_ads = await crud.get_responsible_admin("ads")
    resp_help = await crud.get_responsible_admin("help")
    resp_premium = await crud.get_responsible_admin("premium")
    text = "📢 <b>Reklama va Homiylik sozlamalari</b>\n\n" + (
        "Hozircha tashqi link (Instagram, YouTube va h.k.) qo'shilmagan. "
        "Bular yangi foydalanuvchiga /start'da bir marta ko'rsatiladi." if not links
        else "Linkni o'chirish uchun ustiga bosing."
    )
    if is_super:
        text += "\n\nQuyida — kim reklama/yordam/premium so'rovlari uchun mas'ul ekanini belgilang."
    kb = admin_kb.ads_menu_kb(links, is_super, resp_ads, resp_help, resp_premium)
    return text, kb


@router.message(F.text == "📢 Reklama va Homiylik", IsAdmin())
async def ads_menu(message: Message, state: FSMContext):
    if not await has_permission(message.from_user.id, "ads"):
        await deny(message, "Reklama va Homiylik"); return
    await state.clear()
    text, kb = await _ads_menu_text_and_kb(message.from_user.id)
    await message.answer(text, reply_markup=kb)


@router.callback_query(F.data == "extlink_add")
async def extlink_add_start(callback: CallbackQuery, state: FSMContext):
    if not await has_permission(callback.from_user.id, "ads"):
        await deny(callback, "Reklama va Homiylik"); return
    await callback.message.answer(
        "📝 Link nomini yuboring (masalan: Instagram, YouTube):",
        reply_markup=admin_kb.cancel_kb()
    )
    await state.set_state(AdminStates.waiting_link_title)
    await callback.answer()


@router.message(AdminStates.waiting_link_title)
async def extlink_add_title(message: Message, state: FSMContext):
    if message.text == CANCEL:
        await admin_cancel(message, state); return
    await state.update_data(link_title=message.text.strip())
    await message.answer("🔗 Endi havolani (URL) yuboring:")
    await state.set_state(AdminStates.waiting_link_url)


@router.message(AdminStates.waiting_link_url)
async def extlink_add_url(message: Message, state: FSMContext):
    if message.text == CANCEL:
        await admin_cancel(message, state); return
    data = await state.get_data()
    await crud.add_external_link(data["link_title"], message.text.strip())
    await state.clear()
    await message.answer("✅ Link qo'shildi!", reply_markup=admin_kb.admin_panel_kb())


@router.callback_query(F.data.startswith("extlink_del:"))
async def extlink_del(callback: CallbackQuery):
    if not await has_permission(callback.from_user.id, "ads"):
        await deny(callback, "Reklama va Homiylik"); return
    await crud.delete_external_link(int(callback.data.split(":")[1]))
    text, kb = await _ads_menu_text_and_kb(callback.from_user.id)
    try:
...
@router.callback_query(F.data.startswith("resp_pick:"), IsSuperAdmin())
async def resp_pick_start(callback: CallbackQuery, state: FSMContext):
    role = callback.data.split(":")[1]
    await state.update_data(resp_role=role)
    label = ROLE_LABELS.get(role, role)
    await callback.message.answer(
        f"🆔 '{label}' uchun yangi mas'ul adminning ID sini yuboring "
        f"(u avval 🛡 Adminlar bo'limi orqali admin qilingan bo'lishi kerak):",
        reply_markup=admin_kb.cancel_kb()
    )
    await state.set_state(AdminStates.waiting_responsible_admin_id)
    await callback.answer()


@router.message(AdminStates.waiting_responsible_admin_id, IsSuperAdmin())
async def resp_pick_process(message: Message, state: FSMContext):
    if message.text == CANCEL:
        await admin_cancel(message, state); return
    if not message.text.strip().isdigit():
        await message.answer("❌ Faqat ID (raqam) yuboring."); return
    uid = int(message.text.strip())
    if not await is_admin_async(uid):
        await message.answer("❌ Bu ID admin emas. Avval uni 🛡 Adminlar bo'limidan admin qiling.")
        return
    data = await state.get_data()
    role = data.get("resp_role")
    await crud.set_responsible_admin(role, uid)
    await state.clear()
    label = ROLE_LABELS.get(role, role)
    await message.answer(f"✅ '{label}' uchun mas'ul admin yangilandi!", reply_markup=admin_kb.admin_panel_kb())
    text, kb = await _ads_menu_text_and_kb(message.from_user.id)
    await message.answer(text, reply_markup=kb)
...
async def post_photo(message: Message, state: FSMContext):
    ...
    await state.update_data(post_photo_id=message.photo[-1].file_id, post_caption=caption)
    await message.answer(
        "🎥 Postga video ham qo'shmoqchimisiz? Xohlasangiz videoni yuboring "
        "(rasm + video + matn birga bitta post sifatida joylanadi).\n\n"
        "Kerak bo'lmasa ➡️ Skip bosing:",
        reply_markup=admin_kb.skip_kb()
    )
    await state.set_state(AdminStates.waiting_post_video)
...
async def _publish_post(message: Message, state: FSMContext, bot: Bot, video_file_id: str | None):
    data = await state.get_data()
    code = data.get("post_code")
    channel_ids = data.get("post_channel_ids", [])
    photo_file_id = data.get("post_photo_id")
    ...
    for ch in channels:
        try:
            ...
            if video_file_id:
                # Telegram media-albom (sendMediaGroup) ichidagi elementlarga inline tugma
                # biriktirish MUMKIN EMAS — shu sabab rasm (tavsif + tugma bilan) alohida
                # xabar sifatida, video esa undan keyin oddiy xabar sifatida yuboriladi.
...
@router.message(AdminStates.waiting_post_video, F.video)
async def post_video(message: Message, state: FSMContext, bot: Bot):
    await _publish_post(message, state, bot, message.video.file_id)


@router.message(AdminStates.waiting_post_video)
async def post_video_skip(message: Message, state: FSMContext, bot: Bot):
    if message.text == CANCEL:
        await admin_cancel(message, state); return
    if message.text != SKIP:
        await message.answer("🎥 Iltimos, video yuboring yoki ➡️ Skip bosing.")
        return
    await _publish_post(message, state, bot, None)


# ══════════════════════ 📨 XABARLAR (foydalanuvchi murojaatlari) ══════════════════════

async def _show_feedback_page(target, page: int):
    per_page = 5
    items = await crud.get_feedback_list(page, per_page)
    total = await crud.count_feedback()
    if not items:
        ...
        for fb in items:
            blocks.append(
                f"👤 <b>{fb.full_name}</b> (<code>{fb.tg_id}</code>)\n"
                f"🕒 {fb.created_at.strftime('%Y-%m-%d %H:%M')}\n"
                f"💬 {fb.text}"
            )
        text = f"📨 <b>Murojaatlar</b> (jami {total} ta)\n\n" + "\n\n➖➖➖\n\n".join(blocks)
    kb = admin_kb.feedback_nav_kb(page, total, per_page)
...
@router.message(F.text == "📨 Xabarlar", IsAdmin())
async def feedback_list_start(message: Message, state: FSMContext):
    await state.clear()
    await _show_feedback_page(message, 0)


@router.callback_query(F.data.startswith("fb_page:"))
async def feedback_page_cb(callback: CallbackQuery):
    page = int(callback.data.split(":")[1])
    await _show_feedback_page(callback, page)
    await callback.answer()
