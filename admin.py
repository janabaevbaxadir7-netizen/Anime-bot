...
async def ch_add_id(message: Message, state: FSMContext, bot: Bot):
    ...
    # MUHIM: chat_id ni saqlashdan OLDIN darhol tekshiramiz. Aynan shu tekshiruv
    # yo'qligi "foydalanuvchi obuna bo'lgan bo'lsa ham, bot 'obuna bo'lmagansiz'
    # deb yozadi" degan xatoning asosiy sababi edi: masalan @ belgisisiz yoki
    # t.me/ havolasi ko'rinishida kiritilgan chat_id keyinchalik get_chat_member
    # so'rovida "chat not found" xatosi berib, foydalanuvchi HAR DOIM obuna
    # bo'lmagan deb hisoblanardi — hatto haqiqatda obuna bo'lsa ham.
    try:
        ...
        if member.status not in ("administrator", "creator"):
            await message.answer(
                ...
                f"Majburiy obunani tekshirish uchun bot o'sha kanalda ADMIN bo'lishi SHART "
                f"(bo'lmasa, obuna bo'lgan foydalanuvchilar ham 'obuna emassiz' deb chiqadi).\n\n"
    ...
    await state.update_data(chat_id=normalized, title_hint=chat.title or "")
    await message.answer(f"✅ Kanal topildi: <b>{chat.title or normalized}</b>\n📝 Endi ko'rsatiladigan kanal nomini yuboring:")
...
async def user_lookup_start(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        "🔍 Foydalanuvchi ID sini yoki @username'ini yuboring.\n\n"
        "ℹ️ <b>Agar foydalanuvchida username (nickname) bo'lmasa</b> — "
        "shunchaki o'sha odamning istalgan xabarini shu yerga FORWARD (uzatib) qiling, "
        "bot ID'sini o'zi aniqlab oladi.",
        reply_markup=admin_kb.cancel_kb()
    )
    await state.set_state(AdminStates.waiting_user_query)


async def _present_user_for_premium(message: Message, state: FSMContext, tg_id: int):
    """Topilgan foydalanuvchi haqida ma'lumot ko'rsatadi va Premium kunini so'raydi.
    user_lookup_process (ID/username orqali) va user_lookup_forward (forward orqali)
    ikkalasi ham shu funksiyaga kelib tushadi."""
    user = await crud.get_user(tg_id)
    if not user:
        await message.answer(
            f"⚠️ Bu foydalanuvchi (ID: <code>{tg_id}</code>) botda hali ro'yxatdan o'tmagan "
            f"— demak hech qachon /start bosmagan. Premium berishdan oldin "
            f"undan botga /start bosishini so'rang, keyin qayta urinib ko'ring."
        )
        return
...
@router.message(AdminStates.waiting_user_query, F.forward_from)
async def user_lookup_forward(message: Message, state: FSMContext):
    """Admin nickname'i yo'q foydalanuvchining xabarini FORWARD qilganda ishlaydi —
    ID ni qo'lda qidirishga hojat qoldirmaydi."""
    await _present_user_for_premium(message, state, message.forward_from.id)


@router.message(AdminStates.waiting_user_query, F.forward_sender_name)
async def user_lookup_forward_hidden(message: Message, state: FSMContext):
    """Foydalanuvchi 'Forward qilinganda hisobim ko'rsatilmasin' sozlamasini yoqqan bo'lsa,
    Telegram uning ID/username'ini bermaydi — bunda ID ni boshqa yo'l bilan olish kerak."""
    await message.answer(
        "⚠️ Bu foydalanuvchi maxfiylik sozlamasida ID'sini forward orqali ko'rsatishni "
        "o'chirib qo'ygan. Uning ID'sini olish uchun xabarini @userinfobot ga forward qiling, "
        "so'ng bu yerga o'sha ID raqamini yuboring."
    )
...
async def user_lookup_process(message: Message, state: FSMContext):
    ...
    if not user:
        await message.answer(
            ...
            "ℹ️ Nickname'i yo'q bo'lsa, ID raqamini yuboring yoki shu odamning "
            "xabarini shu yerga forward qiling."
        )
        return
    await _present_user_for_premium(message, state, user.tg_id)
...
async def grant_premium_days(message: Message, state: FSMContext, bot: Bot):
    ...
    days = int(message.text.strip())
    if days <= 0:
        await message.answer("❌ Kunlar soni 0 dan katta bo'lishi kerak."); return
    data = await state.get_data()
    ...
    # MUHIM: tugash sanasini BIR MARTA hisoblab, xuddi shu qiymatni bazaga ham
    # yozamiz, admin va foydalanuvchiga ham shuni ko'rsatamiz — shu orqali
    # "aytilgan kun soni bilan bazadagi/xabardagi sana mos kelmasligi" xavfi yo'qoladi.
    until = datetime.datetime.utcnow() + datetime.timedelta(days=days)
    await crud.set_premium(uid, until)
    await message.answer(
        f"✅ {uid} ga <b>{days} kunlik</b> Premium berildi.\n"
        f"📅 Tugash sanasi: <b>{until.strftime('%Y-%m-%d %H:%M')}</b> (UTC)",
        reply_markup=admin_kb.admin_panel_kb()
    )
    try:
        await bot.send_message(
            uid,
            f"🎉 Sizga <b>{days} kunlik Premium</b> faollashtirildi!\n"
            f"📅 Amal qilish muddati: <b>{until.strftime('%Y-%m-%d')}</b> gacha. Enjoy 💎"
        )
    except Exception:
        pass
