...
def main_menu_kb(is_admin=False):
    ...
    if is_admin:
        kb.append([KeyboardButton(text=texts.MENU_ADMIN_PANEL)])
...
def premium_kb(admin_id=None):
    ...
    if admin_id:
        # tg://user?id= — bevosita ID orqali ochiladi, admin'da @username bo'lmasa ham ishlaydi
        kb.button(text="💬 Admin bilan bog'lanish", url=f"tg://user?id={admin_id}")
    kb.adjust(1)
    return kb.as_markup()


def ads_contact_kb(admin_id=None):
    kb = InlineKeyboardBuilder()
    if admin_id:
        kb.button(text="💬 Admin bilan bog'lanish", url=f"tg://user?id={admin_id}")
    return kb.as_markup()
