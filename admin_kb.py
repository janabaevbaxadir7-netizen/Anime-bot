...
def anime_edit_actions_kb(anime_id):
    ...
    kb.button(text="🎥 Trailer", callback_data=f"aedit_trailer:{anime_id}")
...
# ── 📢 REKLAMA VA HOMIYLIK ──
def ads_menu_kb(links, is_super: bool, resp_ads: int = 0, resp_help: int = 0, resp_premium: int = 0):
    """links — ExternalLink ro'yxati (yangi foydalanuvchiga /start'da ko'rsatiladigan tashqi linklar).
    is_super — faqat bosh adminlarga mas'ul admin tayinlash tugmalari ko'rsatiladi."""
    kb = InlineKeyboardBuilder()
    for l in links:
        kb.button(text=f"🗑 {l.title}", callback_data=f"extlink_del:{l.id}")
    kb.button(text="➕ Link qo'shish", callback_data="extlink_add")
    rows = [1] * len(links) + [1]
    if is_super:
        kb.button(text=f"📢 Reklama mas'uli: {resp_ads}", callback_data="resp_pick:ads")
        kb.button(text=f"☎️ Yordam mas'uli: {resp_help}", callback_data="resp_pick:help")
        kb.button(text=f"💎 Premium mas'uli: {resp_premium}", callback_data="resp_pick:premium")
        rows += [1, 1, 1]
    kb.adjust(*rows)
    return kb.as_markup()
