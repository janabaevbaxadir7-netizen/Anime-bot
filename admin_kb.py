...
def admin_panel_kb() -> ReplyKeyboardMarkup:
    kb = [
        ...
        [KeyboardButton(text="💰 Pul ishlash"), KeyboardButton(text="📨 Xabarlar")],
...
# ── PUL ISHLASH (referal) — ADMIN ──
def earn_admin_menu_kb():
    kb = InlineKeyboardBuilder()
    kb.button(text="📋 Zayavka kanallari", callback_data="earn_ch_list")
    kb.button(text="💳 Yechish so'rovlari", callback_data="wd_list:pending")
    kb.adjust(1)
    return kb.as_markup()


def earn_channels_admin_kb(channels):
    kb = InlineKeyboardBuilder()
    for ch in channels:
        kb.button(text=f"🗑 {ch.title or ch.chat_id} — {ch.reward} so'm", callback_data=f"earn_ch_del:{ch.id}")
    kb.button(text="➕ Kanal qo'shish", callback_data="earn_ch_add")
    kb.adjust(1)
    return kb.as_markup()


def withdraw_list_kb(requests):
    kb = InlineKeyboardBuilder()
    for wr in requests:
        kb.button(text=f"#{wr.id} — {wr.amount} so'm ({wr.tg_id})", callback_data=f"wd_view:{wr.id}")
    kb.adjust(1)
    return kb.as_markup()


def withdraw_view_kb(wr_id):
    kb = InlineKeyboardBuilder()
    kb.button(text="✅ To'landi deb belgilash", callback_data=f"wd_paid:{wr_id}")
    kb.button(text="❌ Rad etish (pul qaytariladi)", callback_data=f"wd_reject:{wr_id}")
    kb.adjust(1)
    return kb.as_markup()
