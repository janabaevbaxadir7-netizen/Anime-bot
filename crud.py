...
from models import User, Anime, Episode, PremiumRequest, Channel, Admin, Feedback, PostChannel, EarnChannel, Referral, WithdrawRequest
...
async def add_earn_channel(chat_id: str, title: str, url: str, reward: int) -> EarnChannel:
    async with async_session() as session:
        ch = EarnChannel(chat_id=chat_id, title=title, url=url, reward=reward)
        session.add(ch)
        await session.commit()
        await session.refresh(ch)
        return ch
...
async def get_all_earn_channels() -> list[EarnChannel]:
    async with async_session() as session:
        result = await session.execute(select(EarnChannel))
        return list(result.scalars().all())


async def delete_earn_channel(channel_id: int):
    async with async_session() as session:
        await session.execute(delete(EarnChannel).where(EarnChannel.id == channel_id))
        await session.commit()


async def set_referred_by(tg_id: int, referrer_id: int) -> bool:
    """Foydalanuvchini birinchi marta kimdir taklif qilgani qayd etiladi.
    O'zini-o'zi taklif qilish yoki qayta yozish oldini olinadi."""
    ...
    async with async_session() as session:
        ...
        if not user or user.referred_by:
            return False
        result2 = await session.execute(select(User).where(User.tg_id == referrer_id))
        if not result2.scalar_one_or_none():
            return False
        result3 = await session.execute(select(Referral).where(Referral.referred_id == tg_id))
        if result3.scalar_one_or_none():
            return False
        user.referred_by = referrer_id
        session.add(Referral(referrer_id=referrer_id, referred_id=tg_id))
        await session.commit()
        return True


async def try_credit_referral(bot, referred_id: int) -> tuple[int, int | None]:
    """referred_id barcha EarnChannel'larga obuna bo'lganmi tekshiradi; agar shunday
    va hali kredit berilmagan bo'lsa — referrer balansiga pul qo'shadi.
    Qaytaradi: (qo'shilgan summa, referrer_id) yoki (0, None)."""
    channels = await get_all_earn_channels()
    if not channels:
        return 0, None
    async with async_session() as session:
        result = await session.execute(select(Referral).where(Referral.referred_id == referred_id))
        ref = result.scalar_one_or_none()
        if not ref or ref.credited:
            return 0, None

        for ch in channels:
            try:
                member = await bot.get_chat_member(ch.chat_id, referred_id)
                if member.status in ("left", "kicked"):
                    return 0, None
            except Exception:
                return 0, None

        total = sum(ch.reward for ch in channels)
        ref.credited = True
        ref.amount = total
        result2 = await session.execute(select(User).where(User.tg_id == ref.referrer_id))
        referrer = result2.scalar_one_or_none()
        if referrer:
            referrer.balance = (referrer.balance or 0) + total
        await session.commit()
        return total, ref.referrer_id
...
async def get_referral_stats(tg_id: int) -> dict:
    async with async_session() as session:
        ...
        total = await session.scalar(select(func.count()).select_from(Referral).where(Referral.referrer_id == tg_id))
        credited = await session.scalar(
            select(func.count()).select_from(Referral).where(Referral.referrer_id == tg_id, Referral.credited == True)
        )
        return {"balance": balance, "total": total or 0, "credited": credited or 0}


async def create_withdraw_request(tg_id: int, amount: int, card_info: str) -> WithdrawRequest | None:
    async with async_session() as session:
        result = await session.execute(select(User).where(User.tg_id == tg_id))
        user = result.scalar_one_or_none()
        if not user or (user.balance or 0) < amount or amount <= 0:
            return None
        user.balance -= amount
        wr = WithdrawRequest(tg_id=tg_id, amount=amount, card_info=card_info, status="pending")
        session.add(wr)
        await session.commit()
...
async def get_withdraw_request(wr_id: int) -> WithdrawRequest | None:
    async with async_session() as session:
        result = await session.execute(select(WithdrawRequest).where(WithdrawRequest.id == wr_id))
        return result.scalar_one_or_none()


async def list_withdraw_requests(status: str = "pending") -> list[WithdrawRequest]:
    async with async_session() as session:
        result = await session.execute(
            select(WithdrawRequest).where(WithdrawRequest.status == status).order_by(WithdrawRequest.created_at)
        )
        return list(result.scalars().all())


async def set_withdraw_status(wr_id: int, status: str) -> WithdrawRequest | None:
    async with async_session() as session:
        result = await session.execute(select(WithdrawRequest).where(WithdrawRequest.id == wr_id))
        ...
        if not wr:
            return None
        wr.status = status
        wr.processed_at = datetime.datetime.utcnow()
        if status == "rejected":
            result2 = await session.execute(select(User).where(User.tg_id == wr.tg_id))
            user = result2.scalar_one_or_none()
            if user:
                user.balance = (user.balance or 0) + wr.amount
        await session.commit()
        await session.refresh(wr)
        return wr
