...
class AdminStates(StatesGroup):
    ...
    waiting_new_trailer = State()           # yangi anime yaratishda trailer video (ixtiyoriy)
    waiting_edit_trailer = State()          # mavjud animening trailerini tahrirlash
    ...
    waiting_post_video = State()            # postga qo'shimcha video (ixtiyoriy)

    # Anime premium sozlash
    waiting_premium_anime_days = State()

    # Reklama va Homiylik (tashqi linklar)
    waiting_link_title = State()
    waiting_link_url = State()

    # Mas'ul adminlarni tayinlash (reklama/yordam/premium)
    waiting_responsible_admin_id = State()
